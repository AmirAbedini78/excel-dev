...
class W:
    ...
    def tool(self,j,n,a,call):
        ...
        if n=="search_parties":return [{"id":4,"code":"CUS-004","name":"فروشگاه پارس الکترونیک"}] if "پارس" in str(a.get("query") or "") else [{"id":3,"code":"CUS-003","name":"کارخانه بهین بسته‌بندی"}]
...
class T(unittest.TestCase):
    ...
    def ctx(self):
        return {"id":100,"company_id":6,"context":{"page_context":{"version":"v1","validated":True,"company_id":6,"source_page":"crm","entities":[{"type":"party","id":3,"code":"CUS-003","label":"کارخانه بهین بسته‌بندی","source_page":"crm"}]}}}
    def test_context_360_skips_name_search(self):
        self.j=self.ctx();t,m=c.process360(self.w,self.j,'وضعیت 360 این مشتری را بده.');self.assertTrue(m["page_context_used"]);self.assertEqual(self.w.calls[0][0],"crm_customer_360");self.assertEqual(self.w.calls[0][1]["party_id"],3)
    def test_context_activity_grounds_party_without_name(self):
        self.j=self.ctx();t,m=c.process_activity(self.w,self.j,'برای این مشتری یک پیگیری با موضوع «تماس Context» برای 1405/06/15 آماده کن');a=[x for x in self.w.calls if x[0]=="create_crm_activity"][0][1];self.assertEqual(a["party_id"],3);self.assertFalse(any(x[0]=="search_parties" for x in self.w.calls))
    def test_context_opportunity_grounds_party_without_name(self):
