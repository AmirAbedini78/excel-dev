...
final class AiToolRegistry
{
    ...
    public static function descriptors(): array
    {
        return [
            ...
            ['name'=>'supplier_performance_summary','mode'=>'read','risk'=>'low','description'=>'Evidence Pack قطعی عملکرد تأمین‌کننده از خرید تاییدشده، رسید انبار و Trade Risk','parameters'=>['type'=>'object','properties'=>['party_ids'=>['type'=>'array','items'=>['type'=>'integer']],'months'=>['type'=>'integer'],'limit'=>['type'=>'integer']]]],
            ['name'=>'shipment_commitment_impact','mode'=>'read','risk'=>'low','description'=>'اثر بالقوه تأخیر پرونده/محموله بر تعهدهای فروش با اتصال Purchase → Item → Inventory → Sales','parameters'=>['type'=>'object','properties'=>['trade_case_id'=>['type'=>'integer'],'shipment_id'=>['type'=>'integer'],'limit'=>['type'=>'integer']]]],
    ...
    public static function executeForWorker(array $job,string $tool,array $args,string $idempotencyKey=''): array
    {
        ...
        return match($tool){
            ...
            'supplier_performance_summary'=>BusinessIntelligenceDomain::supplierPerformanceSummary($wid,$cid,$args),
            'shipment_commitment_impact'=>BusinessIntelligenceDomain::shipmentCommitmentImpact($wid,$cid,$args),
